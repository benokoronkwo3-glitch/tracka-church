...
function ComersPage({ comers, branches, myBranch, church, T, S, Btn, Del, Tag, FL, FG, FC, TH, Grid, onAdd, onDelete, P }) {
  ...
  const printWelcome = comer => {
    const html = `<html><head><title>Welcome - ${comer.name}</title><style>body{font-family:Georgia,serif;padding:40px;max-width:500px;margin:0 auto;color:#333}.header{text-align:center;margin-bottom:30px}.logo{width:80px;height:80px;background:linear-gradient(135deg,#7c2d12,#ea580c,#fbbf24);border-radius:16px;display:flex;align-items:center;justify-content:center;margin:0 auto 16px;font-weight:900;color:#fff;font-size:24px}.church{font-size:18px;font-weight:800;color:#7c2d12}.address{font-size:12px;color:#666;margin-top:4px}.letter{background:#fff7ed;border:2px solid #fed7aa;border-radius:16px;padding:30px;margin-top:20px}.greeting{font-size:22px;color:#ea580c;font-weight:800;text-align:center;margin-bottom:20px}.body{font-size:14px;line-height:1.9;color:#374151}.signature{margin-top:24px;text-align:right}.sig-name{font-weight:800;color:#7c2d12;font-size:15px}.programs{background:#7c2d12;color:#fff;border-radius:10px;padding:16px;margin:16px 0}.programs h3{margin:0 0 8px;font-size:13px;text-transform:uppercase;letter-spacing:1px;color:#fbbf24}.program-item{font-size:13px;padding:4px 0;border-bottom:1px solid #ea580c}</style></head><body>
    <div class="header">
      <div class="logo">GGM</div>
      <div class="church">${church.name}</div>
      <div class="address">${church.address} | ${church.phone}</div>
    </div>
    <div class="letter">
      <div class="greeting">Welcome, ${comer.name}!</div>
      <div class="body">
        Dear ${comer.name},<br/><br/>
        ${church.welcomeMsg}<br/><br/>
        We would love to have you as a permanent member of our family.
      </div>
      <div class="programs">
        <h3>Our Programs - You are invited!</h3>
        <div class="program-item">Sunday Service - Every Sunday</div>
        <div class="program-item">Wednesday Service - Mid-Week</div>
        <div class="program-item">Jesus Clinic - Healing & Miracles</div>
        <div class="program-item">Monday Prayer - Prayer Warriors</div>
        <div class="program-item">Men's Fellowship - For All Men</div>
        <div class="program-item">Women's Fellowship - For All Women</div>
        <div class="program-item">Teen Fellowship - For Teenagers</div>
      </div>
      <div class="body">
        Branch visited: <strong>${comer.branch}</strong><br/>
        Date: <strong>${comer.created_at || tod()}</strong>
      </div>
      <div class="signature">
        <div class="sig-name">${church.pastor}</div>
        <div style="font-size:12px;color:#64748b">(${church.pastoralTitle}) - General Superintendent</div>
        <div style="font-size:12px;color:#64748b">${church.name}</div>
      </div>
    </div>
    <div style="text-align:center;margin-top:20px;font-size:11px;color:#94a3b8">Printed: ${new Date().toLocaleString("en-NG")}</div>
    </body></html>`;
    const w = window.open("", "_blank"); w.document.write(html); w.document.close(); w.print();
  };
