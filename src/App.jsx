...
function MainApp({ church, user, onLogout }) {
  ...
  const printSummary = () => {
    ...
    <h1>${church.name}</h1><p>${church.address} | ${church.phone}<br/>Printed: ${now.toLocaleString("en-NG")}<br/>Branch: ${canSeeAll ? (ab || "All Branches") : user?.branch}</p><hr/>
    ...
    <div style={{ display: "flex", height: "100vh", background: T.light, color: T.dark, fontFamily: "sans-serif", overflow: "hidden" }}>
      <aside style={{ width: 230, background: "#fff", borderRight: `1px solid ${T.border}`, display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        <div style={{ padding: "10px", borderTop: `1px solid ${T.border}` }}>
          ...
          <button onClick={() => setTab("changepin")} style={{ ...S.gBtn, width: "100%", justifyContent: "center", marginBottom: 6, fontSize: 12, padding: "7px 10px" }}>Change My PIN</button>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px", borderBottom: `1px solid ${T.border}`, background: "#fff" }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 800, color: T.dark }}>{tab === "dashboard" ? "God bless you, " + user?.name?.split(" ")[0] : nav.find(n => n.id === tab)?.label || tab}</div>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "16px 20px" }}>
          ...
          {tab === "changepin" && <ChangePinPage user={user} T={T} S={S} onPin={doPin} showToast={t2} onDone={() => setTab("dashboard")} />}
...
function ComersPage({ comers, branches, myBranch, church, T, S, Btn, Del, Tag, FL, FG, FC, TH, Grid, onAdd, onDelete, P }) {
  ...
  const sendWhatsApp = comer => {
    ...
    let phone = (comer.phone || "").toString().trim().replace(/\s+/g, "").replace(/[^0-9]/g, "");
    if (phone.startsWith("0")) phone = "234" + phone.slice(1);
    if (phone.startsWith("+")) phone = phone.slice(1);
    if (!phone.startsWith("234")) phone = "234" + phone;
    const url = phone.length >= 12 ? `https://wa.me/${phone}?text=${msg}` : `https://wa.me/?text=${msg}`;
  ...
  return <div>
    ...
    {open && <FC title="New Comer / First Timer">
      <div style={{ background: T.mid, borderRadius: 8, padding: 12, marginBottom: 14, fontSize: 12, color: T.dark }}>
        A WhatsApp welcome message from <strong>{church.pastor} (Papa)</strong> will be sent automatically.
      </div>
      <FG>
        <FL l="Full Name *"><input style={S.inp} placeholder="Visitor full name" value={form.name} onChange={e => F("name", e.target.value)} /></FL>
      ...
      <button style={{ ...S.save, marginTop: 16 }} onClick={() => {
        ...
        const comer = { ...form };
        onAdd(comer);
        setTimeout(() => sendWhatsApp({ ...comer, created_at: tod() }), 500);
        setForm({ name: "", phone: "", email: "", address: "", gender: "Male", how_heard: "", branch: myBranch || branches[0] || "" });
        setOpen(false);
      }}>Save & Send WhatsApp Welcome</button>
    </FC>}
    <Grid cols={["Name", "Phone", "Gender", "How Heard", "Branch", "Date", "WhatsApp", ...(P.del ? ["Del"] : [])]}
      rows={comers.map(c => [
        ...
        c.created_at || "-",
        <button onClick={() => sendWhatsApp(c)} style={{ ...S.pay, background: "#f0fdf4", color: "#16a34a", border: "1px solid #bbf7d0" }}>WhatsApp</button>,
...
function ReportsPage({ offerings, tithes, expenses, members, attendance, comers, projects, branches, church, ab, canSeeAll, myBranch, T, S, Btn, Tag, TH, Grid, KV, allOfferings, allTithes, allExpenses }) {
  ...
  const exportCSV = () => {
    const rows = [["Type","Details","Amount","Branch","Date"],...fO.map(o=>["Offering",o.service_type+" - "+o.offering_type,o.amount,o.branch,o.date]),...fT.map(t=>["Tithe",t.member_name||"Anonymous",t.amount,t.branch,t.date]),...fE.map(e=>["Expense",e.desc+" ("+e.category+")",e.amount,e.branch,e.date]),...fAt.map(a=>["Attendance",a.description,a.amount,a.branch,a.date]),...fCo.map(c=>["New Comer",c.name+" - "+c.phone,"",c.branch,c.created_at||""])];
  ...
  const doPrint = () => {
    ...
    <h1>${church.name}</h1><p>${church.address} | ${church.phone}<br/>Period: ${pLabel} | Branch: ${branch === "all" ? "All Branches" : branch}<br/>Printed: ${new Date().toLocaleString("en-NG")}</p><hr/>
    <h2>Financial Summary</h2><table><tr><th>Offerings</th><th>Tithes</th><th>Income</th><th>Expenses</th><th>Balance</th></tr><tr><td class="g">${f(totO)}</td><td class="o">${f(totT)}</td><td class="g">${f(totO+totT)}</td><td class="r">${f(totE)}</td><td class="${totO+totT-totE>=0?"g":"r"}">${f(totO+totT-totE)}</td></tr></table>
    <h2>Attendance</h2><table><tr><th>Services</th><th>Total Count</th><th>Average</th><th>New Comers</th></tr><tr><td>${fAt.length}</td><td>${totAt}</td><td>${avgAt}</td><td>${fCo.length}</td></tr></table>
    ${canSeeAll && branch === "all" ? `<h2>Branch Performance</h2><table><thead><tr><th>Branch</th><th>Offerings</th><th>Tithes</th><th>Expenses</th><th>Balance</th><th>Members</th></tr></thead><tbody>${branches.map(b=>{const bO=allOfferings.filter(o=>o.branch===b&&inPeriod(o.date)).reduce((s,r)=>s+ +r.amount,0);const bT=allTithes.filter(t=>t.branch===b&&inPeriod(t.date)).reduce((s,r)=>s+ +r.amount,0);const bE=allExpenses.filter(e=>e.branch===b&&inPeriod(e.date)).reduce((s,r)=>s+ +r.amount,0);const bM=members.filter(m=>m.branch===b).length;return`<tr><td>${b}</td><td class="g">${f(bO)}</td><td class="o">${f(bT)}</td><td class="r">${f(bE)}</td><td class="${bO+bT-bE>=0?"g":"r"}">${f(bO+bT-bE)}</td><td>${bM}</td></tr>`;}).join("")}</tbody></table>` : ""}
    <h2>New Comers (${fCo.length})</h2><table><thead><tr><th>Name</th><th>Phone</th><th>Branch</th><th>Date</th></tr></thead><tbody>${fCo.map(c=>`<tr><td>${c.name}</td><td>${c.phone||"-"}</td><td>${c.branch}</td><td>${c.created_at||""}</td></tr>`).join("")}</tbody></table>
    </body></html>`;
    const w = window.open("", "_blank"); w.document.write(html); w.document.close(); w.print();
  };
  ...
  return <div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(150px,1fr))", gap: 12, marginBottom: 16 }}>
      {[{label:"Offerings",value:fmt(totO),color:"#16a34a"},{label:"Tithes",value:fmt(totT),color:T.primary},{label:"Income",value:fmt(totO+totT),color:"#0369a1"},{label:"Expenses",value:fmt(totE),color:"#ef4444"},{label:"Balance",value:fmt(totO+totT-totE),color:totO+totT-totE>=0?"#16a34a":"#ef4444"},{label:"Members",value:members.length,color:T.primary},{label:"Attend Avg",value:avgAt,color:"#7c3aed"},{label:"New Comers",value:fCo.length,color:"#7c3aed"}].map(k=><div key={k.label} style={{background:"#fff",border:`1px solid ${T.border}`,borderRadius:11,padding:14,borderLeft:`4px solid ${k.color}`}}><div style={{fontSize:11,color:"#374151",marginBottom:4}}>{k.label}</div><div style={{fontSize:17,fontWeight:800,color:k.color}}>{k.value}</div></div>)}
    </div>
    ...
    <div style={card}>
      ...
      <Grid cols={["Date","Service","Branch","Count"]} rows={fAt.map(a=>[a.date,<Tag bg={T.mid} tc={T.primary}>{a.description}</Tag>,a.branch,<strong style={{color:T.primary}}>{a.amount}</strong>])} />
    </div>
    <div style={card}>
      <div style={{ fontWeight: 800, color: "#7c3aed", fontSize: 13, marginBottom: 12, textTransform: "uppercase" }}>New Comers ({fCo.length})</div>
      <Grid cols={["Name","Phone","Branch","Date"]} rows={fCo.map(c=>[<strong style={{color:T.dark}}>{c.name}</strong>,c.phone||"-",<Tag bg={T.mid} tc={T.primary}>{c.branch}</Tag>,c.created_at||"-"])} />
    </div>
  </div>;
}
...
function UsersPage({ users, branches, T, S, Btn, Tag, FL, FG, FC, TH, Grid, Modal, onAdd, onToggle, onPin, showToast }) {
  ...
  return <div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(185px,1fr))", gap: 12, marginBottom: 20 }}>
      {[{role:"pastor",title:"Pastor/Bishop",desc:"Full access - All branches - Delete - Manage users"},{role:"hoa",title:"HOA",desc:"Head of Administration - Financial records"},{role:"auditor",title:"Auditor",desc:"View all branches - Financial reports only"},{role:"accounts",title:"Accounts",desc:"Financial records - Branch specific"},{role:"treasurer",title:"Treasurer",desc:"Financial records - Branch specific"},{role:"secretary",title:"Secretary",desc:"Members - Basic records - Branch specific"},{role:"worker",title:"Worker",desc:"Record offerings and attendance only"}].map(r=>(
...
function ChangePinPage({ user, T, S, onPin, showToast, onDone }) {
  const [oldPin, setOldPin] = useState("");
  const [newPin, setNewPin] = useState("");
  const [confirmPin, setConfirmPin] = useState("");
  const [saving, setSaving] = useState(false);
  const go = async () => {
    if (oldPin !== user.pin) { showToast("Current PIN is incorrect", "error"); return; }
  ...
  return <div style={{ maxWidth: 400 }}>
    <div style={{ fontWeight: 800, fontSize: 18, color: T.dark, marginBottom: 4 }}>Change My PIN</div>
    <div style={{ fontSize: 13, color: "#374151", marginBottom: 24 }}>Logged in as: <strong>{user?.name}</strong></div>
    <div style={{ background: "#fff", border: `1px solid ${T.border}`, borderRadius: 11, padding: 24 }}>
      <div style={{ marginBottom: 16 }}>
        <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: T.primary, marginBottom: 6, textTransform: "uppercase" }}>Current PIN</label>
        <input style={S.inp} type="password" maxLength={8} placeholder="Enter current PIN" value={oldPin} onChange={e => setOldPin(e.target.value)} />
      </div>
      <div style={{ marginBottom: 16 }}>
        <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: T.primary, marginBottom: 6, textTransform: "uppercase" }}>New PIN</label>
        <input style={S.inp} type="password" maxLength={8} placeholder="Enter new PIN (4-8 digits)" value={newPin} onChange={e => setNewPin(e.target.value)} />
      </div>
      <div style={{ marginBottom: 20 }}>
        <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: T.primary, marginBottom: 6, textTransform: "uppercase" }}>Confirm New PIN</label>
        <input style={S.inp} type="password" maxLength={8} placeholder="Repeat new PIN" value={confirmPin} onChange={e => setConfirmPin(e.target.value)} onKeyDown={e => e.key === "Enter" && go()} />
      </div>
      <div style={{ display: "flex", gap: 10 }}>
        <button style={S.canc} onClick={onDone}>Cancel</button>
        <button style={S.save} onClick={go} disabled={saving}>{saving ? "Saving..." : "Change PIN"}</button>
      </div>
    </div>
    <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 12 }}>If you forgot your current PIN contact Bishop to reset it.</div>
  </div>;
         }
